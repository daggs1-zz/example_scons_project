...
def parse_args(target, source, env):
	...
	install_flow = False
	make_targets = ""
	b_type = build_type.type.UNSUPPORTED
	...
	if 'install' in env.Dictionary().keys():
		 ...
		 b_type = env.subst(env['build_type'])
		 implicit = False

	if 'make_targets' in env.Dictionary().keys():
		make_targets = env.subst(str(env['make_targets']))

	return (pkg_name, work_dir, dest_dir, jobs, implicit, install_flow, make_targets, b_type)

def message(target, source, env):
	(pkg_name, work_dir, dest_dir, jobs, implicit, install_flow, make_targets, b_type) = parse_args(target, source, env)
	msg = ""
